...
class Dataset_illumEst_base(myDatasetBase.Dataset_base):
    ...
    def learn_pairs(self, path_list, reduce):
        ...
        for csvnum in range(len(path_list)):
            ...
            path_sphere0 = str(path) + "_sphere"
            path_sphere = pathlib.Path(path_sphere0)
            ...
            for imgnum in range(csv.shape[0]):
                if imgnum % reduce ==0:
                    ...
                    copy(str(path_sphere_spe.joinpath("train", csv[imgnum, 0])), str(path.joinpath("gt")))
